	...
	void SphereVsSphereColliderSystem::FixedUpdate() const
	{
		...
		for (size_t i = 0; i < Size - 1; i++)
		{
			...
			for (size_t j = i + 1; j < Size; j++)
			{
				...
				{
					const glm::vec3 Fi = - (glm::length(RbA->Velocity - RbB->Velocity) * IntersectionDepth.Direction) / (static_cast<float>(FIXED_UPDATE_S) * (RbA->InvMass + 1 / RbB->InvMass));
		...
		for (const auto& [EntP, Plane] : GetEntities<PlaneColliderComponent>())
		{
			for (const auto& [EntS, TransSphere, Sphere, RbSphere] : GetEntities<Transform, SphereColliderComponent, RigidBodyComponent>())
			{
				...
				if (IntersectionDepth.Distance < 0.f && RbSphere)
				{
					...
					Sphere->Pos += NewPos;
					RbSphere->Force -= RbSphere->Gravity / RbSphere->InvMass;
				}
			}
		}
	}
